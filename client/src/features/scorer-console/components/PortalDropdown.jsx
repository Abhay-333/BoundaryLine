  ...
  ({
    ...
    const updatePosition = useCallback(() => {
      ...
      if (align === "start") {
        left1 = rect.left;
        if (
          left1 + dropdownWidth > viewportWidth - 6 &&
          rect.right - dropdownWidth >= 6
        ) {
          left1 = rect.right - dropdownWidth;
        }
      } else {
        // Align right boundary with anchor right boundary
        left1 = rect.right - dropdownWidth;
        // If overflows left side, try aligning to the left boundary
        if (left1 < 6 && rect.left + dropdownWidth <= viewportWidth - 6) {
          left1 = rect.left;
        }
      }

      // Absolute viewport safety clamp (never go out of boundaries)
      left1 = Math.max(6, Math.min(left1, viewportWidth - dropdownWidth - 6));
    ...
    // Outside click handler:dropdown safety exclusions
    useEffect(() => {
      if (!isOpen) return;
